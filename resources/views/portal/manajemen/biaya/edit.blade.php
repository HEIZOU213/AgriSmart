<x-portal-layout portalName="Manajemen Kebun" pageTitle="Edit Biaya" :menuItems="$menuItems">
<div class="max-w-2xl">
    <div class="mb-5 flex items-center gap-3">
        ...
        <div>
            <h2 class="text-xl font-extrabold text-slate-800">Edit Biaya Operasional</h2>
            <p class="text-xs text-slate-500">Perbarui catatan pengeluaran biaya operasional kebun</p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-3xl border border-slate-100 shadow-sm p-6 sm:p-8">
        <form action="{{ route('portal.manajemen.biaya.update', $biaya) }}" method="POST" class="space-y-4">
            @csrf
            @method('PUT')

            <div>
                <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5">Jenis Biaya *</label>
                <input type="text" name="jenis_biaya" value="{{ old('jenis_biaya', $biaya->jenis_biaya) }}" required 
                       ...
                       placeholder="Contoh: Upah Tenaga Kerja, Pembelian Pupuk NPK, Bahan Bakar">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="number" name="jumlah" value="{{ old('jumlah', $biaya->jumlah) }}" min="0" required 
                           class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:border-green-500 focus:ring-2 focus:ring-green-500/20 outline-none text-sm font-semibold text-slate-800">
                ...
                <div>
                    ...
                    <input type="date" name="tanggal" value="{{ old('tanggal', $biaya->tanggal ? $biaya->tanggal->format('Y-m-d') : '') }}" required 
                           class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:border-green-500 focus:ring-2 focus:ring-green-500/20 outline-none text-sm font-semibold text-slate-800">
            ...
            <div>
                ...
                <textarea name="catatan" rows="3" 
                          ...
                          placeholder="Keterangan toko, nomor kuitansi, atau rincian tambahan...">{{ old('catatan', $biaya->catatan) }}</textarea>
            </div>
