<x-portal-layout portalName="Pembibitan" pageTitle="Edit Jadwal Perawatan" :menuItems="$menuItems">
<div class="max-w-2xl">
    <div class="mb-5 flex items-center gap-3">
        <a href="{{ route('portal.pembibitan.jadwal.index') }}" class="text-slate-400 hover:text-slate-600">Kembali</a>
        <h2 class="text-lg font-bold text-slate-800">Edit Jadwal Perawatan Bibit</h2>
    </div>
    <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
        <form action="{{ route('portal.pembibitan.jadwal.update', $jadwal) }}" method="POST" class="space-y-4">
            @csrf
            @method('PUT')
            <div>
                ...
                <x-custom-dropdown 
                    ...
                    :options="$bibitList" 
                    :value="old('bibit_id', $jadwal->bibit_id)" 
                    searchable 
                    required 
                />
            ...
            <div class="flex items-center gap-3 pt-2">
                ...
                <a href="{{ route('portal.pembibitan.jadwal.index') }}" class="px-5 py-3 bg-slate-100 hover:bg-slate-200 text-slate-600 font-bold rounded-xl transition-colors">Batal</a>
            </div>
        </form>
    </div>
</div>
