<x-portal-layout portalName="Pembibitan" pageTitle="Edit Bibit" :menuItems="$menuItems">
<div class="max-w-2xl">
    <div class="mb-5 flex items-center gap-3">
        ...
        <div>
            <h2 class="text-xl font-extrabold text-slate-800">Edit Data Bibit</h2>
            <p class="text-xs text-slate-500">Perbarui rincian identitas dan perkembangan bibit durian</p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-3xl border border-slate-100 shadow-sm p-6 sm:p-8">
        <form action="{{ route('portal.pembibitan.bibit.update', $bibit) }}" method="POST" class="space-y-4">
            @csrf 
            @method('PUT')
            <input type="hidden" name="jumlah" value="{{ $bibit->jumlah ?: 1 }}">

            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5">Kode Tag Bibit *</label>
                    <input type="text" name="kode_bibit" value="{{ old('kode_bibit', $bibit->kode_bibit) }}" required 
                ...
                <div>
                    <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5">Nama Varietas *</label>
                    @php
                        $vOpts = [];
                        foreach(($varietasList ?? []) as $v) { $vOpts[$v] = $v; }
                        if (!empty($bibit->nama_varietas) && !isset($vOpts[$bibit->nama_varietas])) {
                            $vOpts[$bibit->nama_varietas] = $bibit->nama_varietas;
                        }
            ...
            <div>
                ...
                @php
                    ...
                    if (!empty($bibit->asal_benih) && !isset($abOpts[$bibit->asal_benih])) {
                        $abOpts[$bibit->asal_benih] = $bibit->asal_benih;
                    }
                ...
                <x-custom-dropdown 
                    ...
                    :options="$abOpts" 
                    :value="old('asal_benih', $bibit->asal_benih)" 
                    searchable 
                    customWrapId="asal_custom_wrap" 
                    customInputId="asal_custom_input" 
                />
                <div id="asal_custom_wrap" class="mt-2 hidden">
                    <input type="text" name="asal_benih_custom" id="asal_custom_input" 
                           class="w-full px-4 py-2 rounded-xl border border-emerald-300 focus:border-green-500 outline-none text-sm text-slate-800" 
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                    <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5">Status Siklus *</label>
                    <x-custom-dropdown 
                        name="status" 
                        :value="old('status', $bibit->status)" 
                        :options="[
                            'aktif' => 'Aktif',
                            'perawatan' => 'Perawatan',
                ...
                <div>
                    ...
                    <x-custom-dropdown 
                        name="kondisi" 
                        :value="old('kondisi', $bibit->kondisi)" 
                        :options="[
                            'sehat' => 'Sehat',
                ...
                <div>
                    ...
                    <input type="date" name="tanggal_semai" value="{{ old('tanggal_semai', $bibit->tanggal_semai ? $bibit->tanggal_semai->format('Y-m-d') : '') }}" 
                           class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:border-green-500 outline-none text-sm font-semibold text-slate-800">
                </div>
            </div>
            ...
            <div>
                ...
                <textarea name="catatan" rows="3" 
                          ...
                          placeholder="Catatan tambahan...">{{ old('catatan', $bibit->catatan) }}</textarea>
            </div>
            ...
            <div class="flex items-center gap-3 pt-2">
                ...
                <a href="{{ route('portal.pembibitan.bibit.index') }}" class="px-5 py-3 bg-slate-100 hover:bg-slate-200 text-slate-600 font-bold rounded-xl transition-colors">
                    Batal
                </a>
            </div>
        </form>
