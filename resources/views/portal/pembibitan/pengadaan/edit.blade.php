<x-portal-layout portalName="Pembibitan" pageTitle="Edit Pengadaan Bibit" :menuItems="$menuItems">
<div class="max-w-2xl">
    <div class="mb-5 flex items-center gap-3">
        ...
        <div>
            <h2 class="text-xl font-extrabold text-slate-800">Edit Data Pengadaan Bibit</h2>
            <p class="text-xs text-slate-500">Perbarui rincian pasokan bibit durian dari mitra supplier</p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-3xl border border-slate-100 shadow-sm p-6 sm:p-8">
        <form action="{{ route('portal.pembibitan.pengadaan.update', $pengadaan) }}" method="POST" class="space-y-4">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    @php
                        ...
                        if (!empty($pengadaan->nama_supplier) && !isset($supOpts[$pengadaan->nama_supplier])) {
                            $supOpts[$pengadaan->nama_supplier] = $pengadaan->nama_supplier;
                        }
                    ...
                    <x-custom-dropdown 
                        ...
                        :options="$supOpts" 
                        :value="old('nama_supplier', $pengadaan->nama_supplier)" 
                        searchable 
                        customWrapId="supplier_custom_wrap" 
                        customInputId="supplier_custom_input" 
                        required 
                    />
                    <div id="supplier_custom_wrap" class="mt-2 hidden">
                        <input type="text" name="nama_supplier_custom" id="supplier_custom_input" 
                ...
                <div>
                    ...
                    @php
                        ...
                        if (!empty($pengadaan->nama_varietas) && !isset($vOpts[$pengadaan->nama_varietas])) {
                            $vOpts[$pengadaan->nama_varietas] = $pengadaan->nama_varietas;
                        }
                    ...
                    <x-custom-dropdown 
                        ...
                        :options="$vOpts" 
                        :value="old('nama_varietas', $pengadaan->nama_varietas)" 
                        searchable 
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                    ...
                    <input type="number" name="jumlah" value="{{ old('jumlah', $pengadaan->jumlah) }}" min="1" required 
                ...
                <div>
                    ...
                    <input type="number" name="harga_satuan" value="{{ old('harga_satuan', $pengadaan->harga_satuan) }}" min="0" required 
                ...
                <div>
                    ...
                    <input type="date" name="tanggal" value="{{ old('tanggal', $pengadaan->tanggal ? $pengadaan->tanggal->format('Y-m-d') : '') }}" required 
                           class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:border-green-500 outline-none text-sm font-semibold text-slate-800">
            ...
            <div>
                ...
                <textarea name="catatan" rows="3" 
                          ...
                          placeholder="Nomor invoice, batch kedatangan, dll...">{{ old('catatan', $pengadaan->catatan) }}</textarea>
            </div>
            ...
            <div class="flex items-center gap-3 pt-2">
                ...
                <a href="{{ route('portal.pembibitan.pengadaan.index') }}" class="px-5 py-3 bg-slate-100 hover:bg-slate-200 text-slate-600 font-bold rounded-xl transition-colors">
                    Batal
                </a>
            </div>
        </form>
