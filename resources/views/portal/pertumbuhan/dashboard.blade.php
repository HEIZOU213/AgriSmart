...
<x-portal-layout portalName="Pertumbuhan" pageTitle="Dashboard Pertumbuhan" :menuItems="$menuItems">
    ...
    <div class="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
            ...
            <p class="text-emerald-100/90 text-sm mt-1.5 max-w-xl">
                Pantau fase vegetatif hingga masa produktif pohon durian, jadwal perawatan berkala, dan monitoring kesehatan tanaman.
            </p>
        </div>
    ...
    <a href="{{ route('portal.pertumbuhan.pohon.index') }}" 
       class="bg-white rounded-2xl p-5 border border-slate-100 shadow-sm hover:shadow-md hover:border-emerald-200 transition-all group flex flex-col justify-between">
        ...
        <div>
            ...
            <div class="text-xs font-semibold text-slate-500 mt-1">Total Populasi Pohon</div>
        </div>
    </a>

    {{-- 2. Pohon Sehat --}}
    <a href="{{ route('portal.pertumbuhan.pohon.index', ['kondisi' => 'sehat']) }}" 
       class="bg-white rounded-2xl p-5 border border-slate-100 shadow-sm hover:shadow-md hover:border-green-200 transition-all group flex flex-col justify-between">
        ...
        <div>
            <div class="text-2xl sm:text-3xl font-extrabold text-green-700 tracking-tight">{{ number_format($stats['pohon_sehat'], 0, ',', '.') }}</div>
            <div class="text-xs font-semibold text-slate-500 mt-1">Kondisi Sehat Prima</div>
        </div>
    </a>

    {{-- 3. Dalam Perawatan --}}
    <a href="{{ route('portal.pertumbuhan.pohon.index', ['kondisi' => 'perawatan']) }}" 
       class="bg-white rounded-2xl p-5 border border-slate-100 shadow-sm hover:shadow-md hover:border-green-200 transition-all group flex flex-col justify-between">
        <div class="flex items-center justify-between mb-3">
            <div class="w-10 h-10 rounded-xl bg-green-50 text-green-600 flex items-center justify-center group-hover:scale-110 transition-transform">
        ...
        <div>
            <div class="text-2xl sm:text-3xl font-extrabold text-green-600 tracking-tight">{{ number_format($stats['pohon_perawatan'], 0, ',', '.') }}</div>
            <div class="text-xs font-semibold text-slate-500 mt-1">Dalam Perawatan</div>
        </div>
    </a>

    {{-- 4. Pohon Produktif --}}
    <a href="{{ route('portal.pertumbuhan.pohon.index', ['fase' => 'produktif']) }}" 
       class="bg-white rounded-2xl p-5 border border-slate-100 shadow-sm hover:shadow-md hover:border-emerald-200 transition-all group flex flex-col justify-between">
        <div class="flex items-center justify-between mb-3">
            <div class="w-10 h-10 rounded-xl bg-emerald-50 text-emerald-600 flex items-center justify-center group-hover:scale-110 transition-transform">
        ...
        <div>
            <div class="text-2xl sm:text-3xl font-extrabold text-emerald-700 tracking-tight">{{ number_format($stats['pohon_produktif'], 0, ',', '.') }}</div>
            <div class="text-xs font-semibold text-slate-500 mt-1">Fase Produktif (Panen)</div>
        </div>
    </a>

    {{-- 5. Pohon Bermasalah --}}
    <a href="{{ route('portal.pertumbuhan.pohon.index', ['kondisi' => 'bermasalah']) }}" 
       class="bg-white rounded-2xl p-5 border border-slate-100 shadow-sm hover:shadow-md hover:border-red-200 transition-all group flex flex-col justify-between col-span-2 sm:col-span-1">
        <div class="flex items-center justify-between mb-3">
            ...
            <span class="text-xs text-slate-400 group-hover:text-red-600 transition-colors font-semibold">Lihat &rarr;</span>
        </div>
        <div>
            <div class="text-2xl sm:text-3xl font-extrabold text-red-600 tracking-tight">{{ number_format($stats['pohon_bermasalah'], 0, ',', '.') }}</div>
            <div class="text-xs font-semibold text-slate-500 mt-1">Pohon Bermasalah</div>
        </div>
    </a>
</div>
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
    ...
    {{-- KARTU 1: Jadwal Perawatan Perlu Tindakan --}}
    <div class="bg-white rounded-3xl border border-slate-100 shadow-sm p-6 flex flex-col justify-between">
        <div>
            <div class="flex items-center justify-between mb-5">
                ...
                <a href="{{ route('portal.pertumbuhan.jadwal.index') }}" class="text-xs text-emerald-600 font-bold hover:underline">Lihat Semua</a>
            </div>
            ...
            <div class="divide-y divide-slate-100">
                ...
                <div class="py-3.5 flex flex-col sm:flex-row sm:items-center justify-between gap-3 group">
                    <div class="flex items-start gap-3 min-w-0">
                        ...
                        <div class="min-w-0">
                            ...
                            <p class="text-xs text-slate-500 mt-0.5">
                                Pohon: <span class="font-semibold text-slate-700">{{ $j->pohon->nama_varietas ?? 'Pohon #' . $j->pohon_id }}</span> 
                                <span class="font-mono text-slate-400">({{ $j->pohon->kode_pohon ?? '-' }})</span>
                            </p>
                    ...
                    <div class="flex items-center gap-1.5 flex-shrink-0">
                        ...
                        <form action="{{ route('portal.pertumbuhan.jadwal.destroy', $j) }}" method="POST" class="inline" onsubmit="return confirm('Hapus jadwal perawatan pohon ini?')">
                            @csrf @method('DELETE')
                ...
                <div class="text-center py-8">
                    ...
                    <p class="text-xs font-semibold text-slate-500">Tidak ada jadwal perawatan pohon yang tertunda.</p>
                    <a href="{{ route('portal.pertumbuhan.jadwal.create') }}" class="inline-block mt-3 text-xs font-bold text-emerald-600 hover:underline">Buat Jadwal Baru</a>
                </div>
                @endforelse
            </div>
        </div>
        ...
        <div class="mt-4 pt-4 border-t border-slate-100">
            <a href="{{ route('portal.pertumbuhan.jadwal.create') }}" class="w-full py-2.5 rounded-xl border border-dashed border-slate-200 hover:border-emerald-400 text-slate-500 hover:text-emerald-700 text-xs font-bold flex items-center justify-center gap-2 transition-colors">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/></svg>
                Tambah Jadwal Perawatan Pohon
            </a>
        </div>
    </div>

    {{-- KARTU 2: Pohon Perlu Perhatian --}}
    <div class="bg-white rounded-3xl border border-red-100 shadow-sm p-6 flex flex-col justify-between">
        <div>
            <div class="flex items-center justify-between mb-5">
                <div class="flex items-center gap-2.5">
                    ...
                    <div>
                        <h3 class="font-bold text-slate-800 text-base">Pohon Perlu Perhatian</h3>
                        <p class="text-xs text-red-500 font-medium">Kondisi bermasalah atau dalam masa pemulihan</p>
                    </div>
                </div>
                <a href="{{ route('portal.pertumbuhan.pohon.index', ['kondisi' => 'bermasalah']) }}" class="text-xs text-red-600 font-bold hover:underline">Kelola Data</a>
            </div>

            <div class="divide-y divide-slate-100">
                @forelse($pohonPerhatian as $p)
                <div class="py-3.5 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                    <div class="flex items-start gap-3 min-w-0">
                        <span class="mt-1.5 w-2.5 h-2.5 rounded-full flex-shrink-0 {{ $p->kondisi === 'bermasalah' ? 'bg-red-500' : 'bg-amber-500' }}"></span>
                        <div class="min-w-0">
                            <p class="text-sm font-bold text-slate-800 truncate">{{ $p->nama_varietas }}</p>
                            <p class="text-xs text-slate-500 mt-0.5">
                                Kode: <span class="font-mono text-slate-700 font-bold">{{ $p->kode_pohon }}</span> 
                                &bull; Lahan: <span class="font-semibold text-slate-700">{{ $p->lahan->nama_lahan ?? ($p->lokasi ?? 'Kebun Utama') }}</span>
                            </p>
                            <div class="mt-1 flex items-center gap-1.5">
                                <span class="px-2 py-0.5 rounded-md text-[10px] font-extrabold {{ $p->kondisi === 'bermasalah' ? 'bg-red-100 text-red-700' : 'bg-amber-100 text-amber-700' }}">
                                    {{ ucfirst($p->kondisi) }}
                                </span>
                    ...
                    <div class="flex items-center gap-1.5 flex-shrink-0">
                        <a href="{{ route('portal.pertumbuhan.monitoring.create', ['pohon_id' => $p->id]) }}" 
                           class="px-2.5 py-1.5 bg-green-50 hover:bg-green-600 text-green-700 hover:text-white rounded-xl text-xs font-bold transition-all shadow-sm" title="Catat Monitoring untuk pohon ini">
                            Monitoring
                        </a>
                        ...
                        <form action="{{ route('portal.pertumbuhan.pohon.destroy', $p) }}" method="POST" class="inline" onsubmit="return confirm('Hapus data pohon ini?')">
                            @csrf @method('DELETE')
                ...
                <div class="text-center py-8">
                    ...
                    <p class="text-xs font-bold text-slate-700">Kondisi Kebun Sangat Baik</p>
                    <p class="text-xs text-slate-400 mt-0.5">Tidak ada pohon yang bermasalah atau sakit.</p>
                </div>
                @endforelse
            </div>
        </div>
        ...
        <div class="mt-4 pt-4 border-t border-slate-100">
            <a href="{{ route('portal.pertumbuhan.monitoring.create') }}" class="w-full py-2.5 rounded-xl border border-dashed border-slate-200 hover:border-green-400 text-slate-500 hover:text-green-700 text-xs font-bold flex items-center justify-center gap-2 transition-colors">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                Catat Pemeriksaan Monitoring Pohon
            </a>
        </div>
    </div>

    {{-- KARTU 3: Monitoring Perkembangan Terbaru --}}
    <div class="bg-white rounded-3xl border border-slate-100 shadow-sm p-6 flex flex-col justify-between">
        <div>
            <div class="flex items-center justify-between mb-5">
                <div class="flex items-center gap-2.5">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-slate-400">Pengukuran fisik & kondisi berkala</p>
                    </div>
                </div>
                <a href="{{ route('portal.pertumbuhan.monitoring.index') }}" class="text-xs text-green-600 font-bold hover:underline">Lihat Semua</a>
            </div>

            <div class="divide-y divide-slate-100">
                @forelse($monitoringTerbaru as $m)
                <div class="py-3 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                    <div class="flex items-start gap-3 min-w-0">
                        <span class="mt-1.5 w-2.5 h-2.5 rounded-full flex-shrink-0 {{ $m->kondisi === 'sehat' ? 'bg-emerald-500' : ($m->kondisi === 'bermasalah' ? 'bg-red-500' : 'bg-amber-400') }}"></span>
                        <div class="min-w-0">
                            <p class="text-sm font-bold text-slate-800 truncate">
                                {{ $m->pohon->nama_varietas ?? 'Pohon #' . $m->pohon_id }}
                                <span class="text-xs font-mono text-slate-400">({{ $m->pohon->kode_pohon ?? '-' }})</span>
                            </p>
                            <p class="text-xs text-slate-500 mt-0.5">
                                Kondisi: <span class="font-semibold text-slate-700 capitalize">{{ $m->kondisi }}</span>
                                &bull; {{ $m->tanggal ? $m->tanggal->format('d M Y') : '-' }}
                            </p>
                            @if($m->catatan)
                    ...
                    <div class="flex items-center gap-2 flex-shrink-0">
                        <div class="text-right">
                            @if($m->tinggi_cm)
                                <span class="inline-flex items-center px-2 py-0.5 rounded-lg text-xs font-extrabold bg-slate-50 text-slate-700 border border-slate-100">
                                    {{ $m->tinggi_cm }} cm
                                </span>
                            @endif
                            @if($m->diameter_batang)
                                <span class="inline-flex items-center px-2 py-0.5 rounded-lg text-[10px] font-bold bg-slate-50 text-slate-500 border border-slate-100">
                        ...
                        <form action="{{ route('portal.pertumbuhan.monitoring.destroy', $m) }}" method="POST" class="inline" onsubmit="return confirm('Hapus data monitoring ini?')">
                            @csrf @method('DELETE')
                ...
                @empty
                <div class="text-center py-8">
                    <p class="text-xs text-slate-400">Belum ada riwayat monitoring pohon.</p>
                </div>
                @endforelse
            </div>
        </div>
        ...
        <div class="mt-4 pt-4 border-t border-slate-100">
            <a href="{{ route('portal.pertumbuhan.monitoring.create') }}" class="text-xs font-bold text-green-600 hover:text-green-700 flex items-center justify-center gap-1.5">
                Input Data Monitoring Pohon Baru
            </a>
        </div>
    </div>

    {{-- KARTU 4: Distribusi Fase Pertumbuhan --}}
    <div class="bg-white rounded-3xl border border-slate-100 shadow-sm p-6 flex flex-col justify-between">
        <div>
            <div class="flex items-center justify-between mb-5">
                <div class="flex items-center gap-2.5">
                    ...
                    <div>
                        <h3 class="font-bold text-slate-800 text-base">Distribusi Fase Pohon</h3>
                        <p class="text-xs text-slate-400">Klasifikasi tahap pertumbuhan tanaman</p>
                    </div>
                </div>
                <a href="{{ route('portal.pertumbuhan.fase') }}" class="text-xs text-green-600 font-bold hover:underline">Buka Detail Fase</a>
            </div>

            @php
                $faseColors = [
                    'bibit'      => ['bg' => 'bg-emerald-50', 'text' => 'text-emerald-700', 'bar' => 'bg-emerald-500', 'desc' => 'Pohon muda awal tanam'],
                    'vegetatif'  => ['bg' => 'bg-emerald-50', 'text' => 'text-emerald-700', 'bar' => 'bg-emerald-500', 'desc' => 'Pertumbuhan dahan & daun'],
                    'generatif'  => ['bg' => 'bg-green-50', 'text' => 'text-green-700', 'bar' => 'bg-green-500', 'desc' => 'Muncul bunga & bakal buah'],
                    'produktif'  => ['bg' => 'bg-green-50', 'text' => 'text-green-700', 'bar' => 'bg-green-500', 'desc' => 'Siap panen rutin durian'],
                ];
                $totalAll = max(1, $stats['total_pohon']);
            @endphp

            <div class="space-y-3.5">
                @foreach(['bibit', 'vegetatif', 'generatif', 'produktif'] as $fKey)
                    @php
                        $count = $faseSummary->firstWhere('fase', $fKey)->total ?? 0;
                        $pct = round(($count / $totalAll) * 100);
                        $cfg = $faseColors[$fKey];
                    @endphp
                    <a href="{{ route('portal.pertumbuhan.pohon.index', ['fase' => $fKey]) }}" 
                       class="block p-3 rounded-2xl hover:bg-slate-50 transition-colors border border-transparent hover:border-slate-100 group">
                        <div class="flex items-center justify-between text-xs mb-1.5">
                            <span class="font-bold text-slate-800 group-hover:text-emerald-700 capitalize flex items-center gap-1.5">
                                <span class="w-2 h-2 rounded-full {{ $cfg['bar'] }}"></span>
                                {{ ucfirst($fKey) }}
                                <span class="text-[11px] font-normal text-slate-400">({{ $cfg['desc'] }})</span>
                            </span>
                            <span class="font-extrabold text-slate-900">{{ $count }} pohon ({{ $pct }}%)</span>
                        </div>
        ...
        <div class="mt-4 pt-4 border-t border-slate-100 flex items-center justify-between">
            <a href="{{ route('portal.pertumbuhan.pohon.create') }}" class="text-xs font-bold text-emerald-600 hover:text-emerald-700 flex items-center gap-1">
                Tambah Pohon Baru
            </a>
            <a href="{{ route('portal.pertumbuhan.laporan') }}" class="text-xs font-bold text-slate-500 hover:text-slate-800 flex items-center gap-1">
                Buka Laporan Pertumbuhan &rarr;
            </a>
        </div>
    </div>
</div>
...
<div class="bg-white rounded-3xl border border-slate-100 shadow-sm p-6 sm:p-8">
    <div class="flex items-center justify-between mb-6">
        <div>
            ...
            <p class="text-xs text-slate-400 mt-0.5">Menu pintasan untuk alur kerja operasional pertumbuhan kebun durian</p>
        </div>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
        ...
        <a href="{{ route('portal.pertumbuhan.pohon.create') }}" 
           class="p-5 rounded-2xl bg-green-50/60 hover:bg-green-100/60 border border-green-100 transition-all group flex flex-col justify-between">
            ...
            <div>
                <span class="block text-sm font-extrabold text-slate-900 group-hover:text-green-800">Tambah Pohon</span>
                <span class="text-[11px] text-slate-500 mt-0.5 block">Registrasi pohon durian baru</span>
            </div>
        </a>
        ...
        <a href="{{ route('portal.pertumbuhan.monitoring.create') }}" 
           class="p-5 rounded-2xl bg-green-50/60 hover:bg-green-100/60 border border-green-100 transition-all group flex flex-col justify-between">
            ...
            <div>
                ...
                <span class="text-[11px] text-slate-500 mt-0.5 block">Catat tinggi, diameter & cabang</span>
            </div>
        </a>
        ...
        <a href="{{ route('portal.pertumbuhan.jadwal.create') }}" 
           class="p-5 rounded-2xl bg-green-50/60 hover:bg-green-100/60 border border-green-100 transition-all group flex flex-col justify-between">
            ...
            <div>
                ...
                <span class="text-[11px] text-slate-500 mt-0.5 block">Jadwalkan pupuk & pangkas</span>
            </div>
        </a>

        {{-- 4. Fase Pertumbuhan --}}
        <a href="{{ route('portal.pertumbuhan.fase') }}" 
           class="p-5 rounded-2xl bg-green-50/60 hover:bg-green-100/60 border border-green-100 transition-all group flex flex-col justify-between">
            <div class="w-12 h-12 rounded-xl bg-white text-green-600 flex items-center justify-center shadow-sm mb-4 group-hover:scale-110 transition-transform">
                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10"/></svg>
            </div>
            <div>
                <span class="block text-sm font-extrabold text-slate-900 group-hover:text-green-800">Fase Pohon</span>
                <span class="text-[11px] text-slate-500 mt-0.5 block">Lihat pohon per tahap umur</span>
            </div>
        </a>

        {{-- 5. Laporan & Statistik --}}
        <a href="{{ route('portal.pertumbuhan.laporan') }}" 
           class="p-5 rounded-2xl bg-green-50/60 hover:bg-green-100/60 border border-green-100 transition-all group flex flex-col justify-between col-span-2 sm:col-span-1">
            ...
            <div>
                ...
                <span class="text-[11px] text-slate-500 mt-0.5 block">Statistik rekap fase & kondisi</span>
            </div>
        </a>
    </div>
</div>
