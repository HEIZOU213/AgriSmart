    ...
    <div class="text-gray-900">
        ...
        <div class="bg-white rounded-2xl border border-slate-200 p-5 sm:p-7 shadow-sm">
            <form action="{{ route('petani.produk.update', $produk->id) }}" method="POST" enctype="multipart/form-data"
                  x-data="{ 
                      tipe: '{{ old('tipe_produk', $produk->tipe_produk ?? 'ready_stock') }}',
                      estimasi: '{{ old('estimasi_panen', $produk->estimasi_panen ?? '') }}'
                  }">
                @csrf 
                @method('PUT')
                ...
                <div class="space-y-6">
                        ...
                        <div class="flex items-start gap-2.5">
                            ...
                            <div>
                                ...
                                <p class="text-xs text-amber-800 mt-0.5 leading-relaxed">
                                    Konsumen membayar Down Payment (DP) tetap <strong>Rp 100.000</strong> per pemesanan. Saat panen raya tiba, pekebun mengukur timbangan aktual lalu konsumen melunasi via scan QR kuitansi.
                                </p>
                            </div>
                        </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        ...
                        <div>
                            ...
                            <select id="satuan" name="satuan"
                                ...
                                <option value="kg" {{ old('satuan', $produk->satuan ?? 'kg') == 'kg' ? 'selected' : '' }}>Kg (Kilogram)</option>
                                <option value="buah" {{ old('satuan', $produk->satuan) == 'buah' ? 'selected' : '' }}>Buah</option>
                                <option value="pohon" {{ old('satuan', $produk->satuan) == 'pohon' ? 'selected' : '' }}>Pohon</option>
                                <option value="pcs" {{ old('satuan', $produk->satuan) == 'pcs' ? 'selected' : '' }}>Pcs / Pack</option>
                                <option value="bibit" {{ old('satuan', $produk->satuan) == 'bibit' ? 'selected' : '' }}>Bibit</option>
                            </select>
                            @error('satuan')
                                <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                            @enderror
                    ...
                    <div>
                        ...
                        <textarea name="deskripsi" id="deskripsi" rows="4" 
                                  class="block w-full border-gray-300 rounded-xl shadow-sm focus:border-green-500 focus:ring-green-500 text-sm p-3.5">{{ old('deskripsi', $produk->deskripsi) }}</textarea>
                        @error('deskripsi')
                            <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                        @enderror
                    </div>

                    {{-- 7. Foto Produk Saat Ini & Ganti Foto --}}
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 mb-2">Foto Produk</label>
                        @if($produk->foto_produk)
                            <div class="mb-3 flex items-center gap-3">
                                <img src="{{ asset('storage/' . $produk->foto_produk) }}" alt="{{ $produk->nama_produk }}" class="w-24 h-24 object-cover rounded-xl border border-slate-200 shadow-sm">
                                <span class="text-xs text-slate-500">Foto saat ini aktif di etalase toko.</span>
                            </div>
                        @else
                            <p class="mb-2 text-xs text-slate-400 italic">Belum ada foto produk.</p>
                        @endif

                        <label for="foto_produk" class="block text-xs font-semibold text-slate-600 mb-1">Ganti Foto Produk (Opsional, Maks. 3 MB)</label>
                        ...
                        <small class="mt-1 block text-[11px] text-slate-400">Biarkan kosong jika tidak ingin mengubah foto produk.</small>
                        @error('foto_produk')
                            <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
