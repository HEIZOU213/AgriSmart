...
<body class="font-sans antialiased bg-gray-50 text-slate-800">
    ...
    @if(Auth::user()->role === 'konsumen')
        ...
        {{-- >>>>>>>>>>>>>>>>>> NAVBAR KHUSUS KONSUMEN (Alpine.js) <<<<<<<<<<<<<<<<<< --}}
    ...
    <nav x-data="{ mobileOpen: false, scrolled: false, dropdownOpen: false }"
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 transition-all duration-300">
            <div class="flex items-center justify-between h-16 lg:h-20">
                ...
                <a href="/" class="flex items-center gap-2 group relative shrink-0">
                    ...
                    <div class="hidden lg:flex flex-none items-center justify-center gap-6">
                        <a href="{{ Route::has('konsumen.pesanan.index') ? route('konsumen.pesanan.index') : '#' }}" class="relative px-3 py-2 rounded-lg font-semibold text-slate-600 hover:text-green-700 transition-all duration-300 group">
                            Pesanan Saya
                            <span class="absolute bottom-0 left-1/2 -translate-x-1/2 h-[2px] bg-green-600 transition-all duration-300 rounded-full w-0 group-hover:w-3/4"></span>
                        </a>
                        <a href="{{ url('/chat') }}" class="relative px-3 py-2 rounded-lg font-semibold text-slate-600 hover:text-green-700 transition-all duration-300 group">
                            Pesan
                        </a>
                    </div>
                    ...
                    <div class="flex-1 flex justify-end items-center gap-2 sm:gap-4">
                        <a href="{{ Route::has('cart.index') ? route('cart.index') : '#' }}" class="group relative p-2 text-slate-600 hover:text-green-700 transition-colors hidden sm:block mr-1">
                            <svg class="w-6 h-6 transition-transform group-hover:scale-110" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z" /></svg>
                             ...
                             <span id="badge-cart-desktop" class="hidden absolute top-0 right-0 -mt-1 -mr-1 flex h-5 w-5 items-center justify-center rounded-full bg-red-600 text-[10px] font-bold text-white shadow-md">0</span>
                        </a>
            ...
            <div x-show="mobileOpen" class="lg:hidden fixed inset-x-0 top-[64px] bg-white border-t border-green-100 shadow-xl z-40 max-h-[80vh] overflow-y-auto" style="display: none;">
                <div class="px-6 py-6 space-y-2">
                    <a href="{{ Route::has('konsumen.pesanan.index') ? route('konsumen.pesanan.index') : '#' }}" class="block py-3 px-4 font-semibold text-slate-700 hover:bg-green-50 rounded-xl">Pesanan Saya</a>
                    <a href="{{ url('/chat') }}" class="block py-3 px-4 font-semibold text-slate-700 hover:bg-green-50 rounded-xl">Pesan/chat <span id="badge-chat-mobile" class="hidden bg-red-600 text-white text-xs px-2 py-1 rounded-full ml-2">0</span></a>
                    <a href="{{ Route::has('cart.index') ? route('cart.index') : '#' }}" class="block py-3 px-4 font-semibold text-slate-700 hover:bg-green-50 rounded-xl">Keranjang <span id="badge-cart-mobile" class="hidden bg-red-600 text-white text-xs px-2 py-1 rounded-full ml-2">0</span></a>
                    <div class="border-t border-gray-100 my-2 pt-2">
                         <form method="POST" action="{{ route('logout') }}">@csrf<button class="block w-full text-left py-3 px-4 font-medium text-red-600">Keluar</button></form>
                    </div>
                </div>
            </div>
        </nav>
    ...
    @else
        ...
        <nav class="fixed top-0 left-0 right-0 z-50 bg-white border-b border-gray-200 h-16 flex items-center justify-between px-4 md:px-6 shadow-sm">
            <div class="flex items-center gap-2">
                <a href="/" class="flex items-center gap-2 group relative shrink-0">
                    <img src="{{ asset('images/nav-logo.png') }}" alt="AgriSmart Logo" class="h-36 lg:h-40 w-auto object-contain transition-transform duration-300 group-hover:scale-105">
                </a>
            </div>
            <div class="flex items-center gap-3">
                <div class="relative">
                    <button id="adminUserBtn" class="flex items-center gap-2 focus:outline-none group">
                        ...
                        @else
                            <div class="h-9 w-9 rounded-full bg-indigo-100 flex items-center justify-center text-indigo-600 font-bold ring-2 ring-gray-200 group-hover:ring-indigo-500 transition">{{ substr(Auth::user()->name, 0, 1) }}</div>
                        @endif
                    ...
                    <div id="adminUserDropdown" class="hidden absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-lg border border-gray-100 py-1 z-50">
                        @if(Auth::user()->role === 'admin')
                            <a href="{{ route('admin.dashboard') }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-50 hover:text-indigo-600">Dashboard Admin</a>
                        @elseif(Auth::user()->role === 'petani')
                            <a href="{{ route('petani.dashboard') }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-50 hover:text-green-600">Dashboard Petani</a>
                        @endif
    ...
    <main class="pt-24 pb-12 max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
             ...
             <div class="md:hidden">
                @if(Auth::user()->role === 'admin')
                    <a href="{{ route('admin.dashboard') }}" class="inline-flex items-center text-sm font-medium text-indigo-600 hover:text-indigo-800">&larr; Kembali ke Dashboard</a>
                @elseif(Auth::user()->role === 'petani')
                    <a href="{{ route('petani.dashboard') }}" class="inline-flex items-center text-sm font-medium text-green-600 hover:text-green-800">&larr; Kembali ke Dashboard</a>
                @endif
             </div>
        </div>
        ...
        @if (session('status') === 'profile-updated')
            <div class="p-4 bg-green-50 border-l-4 border-green-500 text-green-700 rounded-r-lg shadow-sm flex items-center">
                ...
                <div><p class="font-bold">Berhasil!</p><p class="text-sm">Informasi profil Anda telah diperbarui.</p></div>
            </div>
        @endif
        @if (session('status') === 'password-updated')
            <div class="p-4 bg-green-50 border-l-4 border-green-500 text-green-700 rounded-r-lg shadow-sm flex items-center">
                <svg class="w-6 h-6 mr-3 text-green-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                 <div><p class="font-bold">Berhasil!</p><p class="text-sm">Kata sandi Anda telah diperbarui.</p></div>
            </div>
        @endif
                ...
                <form method="post" action="{{ route('profile.update') }}" class="space-y-6" enctype="multipart/form-data">
                    ...
                    <div class="flex flex-col sm:flex-row items-start sm:items-center gap-6 pb-6 border-b border-gray-100">
                        ...
                        <div class="flex-1 w-full">
                            ...
                            <input type="file" name="foto_profil" class="block w-full text-sm text-gray-500 file:mr-4 file:py-2.5 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-bold file:bg-green-50 file:text-green-700 hover:file:bg-green-100 cursor-pointer transition"/>
                        ...
                        <div>
                            <label for="email" class="block text-sm font-bold text-gray-700 mb-2">Email</label>
                             <div class="relative">
                                ...
                                <input id="email" name="email" type="email" class="w-full pl-10 pr-4 py-2.5 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent transition" value="{{ old('email', $user->email) }}" required>
                            </div>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                <svg class="h-5 w-5 text-gray-400" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor"><path d="M2 3a1 1 0 011-1h2.153a1 1 0 01.986.836l.74 4.435a1 1 0 01-.54 1.06l-1.548.773a11.037 11.037 0 006.105 6.105l.774-1.548a1 1 0 011.059-.54l4.435.74a1 1 0 01.836.986V17a1 1 0 01-1 1h-2C7.82 18 2 12.18 2 5V3z" /></svg>
                            </div>
