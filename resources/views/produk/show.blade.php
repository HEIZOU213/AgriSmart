        ...
        <div class="max-w-7xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8 relative z-10">
            ...
            <div class="mb-4 md:mb-6">
                <nav class="flex items-center text-sm font-medium text-slate-500">
                    ...
                    <a href="{{ route('produk.index') }}"
                        class="group flex items-center hover:text-green-600 transition-colors duration-200 p-1 md:p-0">
                        <span class="back-button-text md:inline">Daftar Produk</span>
                    </a>

                    {{-- Separator (Chevron) --}}
                    <svg class="w-4 h-4 mx-2 md:mx-3 text-slate-300 breadcrumb-separator" fill="none"
                    ...
                    <span
                        class="text-green-600 font-semibold truncate max-w-[120px] xs:max-w-[180px] sm:max-w-md bg-green-50 px-2 py-0.5 rounded-md text-xs md:text-sm"
                        aria-current="page">
                        {{ $produk->nama_produk }}
                    </span>
                </nav>
            ...
            @if (session('success'))
                <div x-data="{ show: true }" x-show="show"
                    class="mb-4 md:mb-6 p-3 md:p-4 bg-green-100 border border-green-200 text-green-800 rounded-xl md:rounded-2xl flex justify-between items-center shadow-sm">
                    <div class="flex items-center">
                        <svg class="w-5 h-5 md:w-6 md:h-6 mr-2 md:mr-3 text-green-600" fill="none" stroke="currentColor"
                            viewBox="0 0 24 24">
            ...
            @if (session('error'))
                <div x-data="{ show: true }" x-show="show"
                    class="mb-4 md:mb-6 p-3 md:p-4 bg-red-50 border border-red-200 text-red-800 rounded-xl md:rounded-2xl flex justify-between items-center shadow-sm">
                    <div class="flex items-center">
                        <svg class="w-5 h-5 md:w-6 md:h-6 mr-2 md:mr-3 text-red-500" fill="none" stroke="currentColor"
                            viewBox="0 0 24 24">
            ...
            {{-- MAIN PRODUCT CARD --}}
            <div
                class="bg-white rounded-2xl md:rounded-3xl border border-slate-200 md:border-slate-300 overflow-hidden shadow-sm md:shadow-md">
                <div class="product-grid grid grid-cols-1 md:grid-cols-12 gap-0 md:gap-6 lg:gap-8">
                    ...
                    <div class="md:col-span-5 bg-slate-100 relative group h-full product-image-container">
                        ...
                        <div class="relative w-full h-full min-h-[250px] sm:min-h-[300px] md:min-h-full">
                            ...
                            @else
                                <div
                                    class="w-full h-full flex flex-col items-center justify-center text-slate-400 bg-slate-100 absolute inset-0">
                    ...
                    <div class="md:col-span-7 p-4 md:p-6 lg:p-8 xl:p-10 flex flex-col justify-between product-details">
                        <div>
                            {{-- Header Title & Price --}}
                            <div
                                class="flex flex-col sm:flex-row justify-between items-start gap-3 md:gap-4 mb-4 md:mb-6">
                            ...
                            <div
                                class="bg-white border border-slate-200 rounded-xl p-3 md:p-4 mb-4 md:mb-6 shadow-sm hover:shadow-md transition-shadow seller-info">
                                <div class="flex items-center gap-3 md:gap-4">
                                    ...
                                    <div class="flex-shrink-0">
                                        ...
                                        @else
                                            <div
                                                class="h-12 w-12 md:h-14 md:w-14 rounded-full bg-green-100 flex items-center justify-center text-green-600 border-2 border-white">
                                                <svg class="h-6 w-6 md:h-7 md:w-7" fill="none" stroke="currentColor"
                                                    viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    ...
                                    <div class="min-w-0 flex-1">
                                        <p class="text-sm md:text-base font-bold text-slate-900 truncate">
                                            {{ $produk->user->name ?? 'AgriSmart Member' }}
                                        </p>
                                        <div class="flex items-center mt-1 text-xs md:text-sm text-slate-500">
                                            <svg class="w-3 h-3 md:w-4 md:h-4 text-green-500 mr-1.5 flex-shrink-0"
                                            ...
                                            <span
                                                class="truncate">{{ $produk->user->alamat ?? 'Lokasi tidak tersedia' }}</span>
                                        </div>
                                    </div>
                                    ...
                                    <div class="flex-shrink-0">
                                        @if(Auth::id() !== $produk->user_id)
                                            {{-- Tombol Chat: Controller akan otomatis mengatur Back URL ke halaman produk
                                            ini --}}
                                            <a href="{{ route('chat.product', $produk->id) }}"
                                                class="inline-flex items-center justify-center p-2 md:px-4 md:py-2 border border-green-200 shadow-sm text-xs md:text-sm font-medium rounded-lg text-green-700 bg-green-50 hover:bg-green-100 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 transition-colors">
                                                <svg class="w-4 h-4 md:mr-2" fill="none" stroke="currentColor"
                                                    viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                        d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z">
                                                    </path>
                                                </svg>
                                                <span class="hidden md:inline">Chat Penjual</span>
                                            </a>
                                        @else
                                            {{-- Tombol Disabled jika melihat produk sendiri --}}
                                            <button disabled
                                                class="inline-flex items-center justify-center p-2 md:px-4 md:py-2 border border-slate-200 shadow-sm text-xs md:text-sm font-medium rounded-lg text-slate-400 bg-slate-50 cursor-not-allowed">
                                                <svg class="w-4 h-4 md:mr-2" fill="none" stroke="currentColor"
                                                    viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                        d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z">
                                                ...
                                                <span class="hidden md:inline">Produk Anda</span>
                                            </button>
                                        @endif
                                    </div>
