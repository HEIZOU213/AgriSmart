    ...
    <x-slot name="header">
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
                ...
                <p class="text-sm text-gray-500 mt-1">Perbarui informasi, artikel, atau video edukasi pertanian.</p>
            </div>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div class="lg:col-span-2 space-y-6">
                        <div class="bg-white overflow-hidden shadow-xl sm:rounded-2xl border border-gray-100">
                            <div class="p-6 sm:p-8 space-y-6">
                                ...
                                {{-- Input Judul --}}
                                <div>
                                    <label for="judul" class="block text-sm font-semibold text-gray-700 mb-2">Judul Konten</label>
                                    <input type="text" id="judul" name="judul" value="{{ old('judul', $konten->judul) }}" 
                                           class="w-full border-gray-300 rounded-xl shadow-sm focus:border-emerald-500 focus:ring-emerald-500 py-3 px-4 text-gray-700 transition duration-200 placeholder-gray-400"
                                           placeholder="Contoh: Cara Menanam Cabai Rawit Anti Gagal">
                                </div>

                                {{-- Input Isi Konten (Textarea) --}}
                                <div>
                                    <label for="isi_konten" class="block text-sm font-semibold text-gray-700 mb-2">Isi Artikel</label>
                                    <div class="relative">
                                        <textarea name="isi_konten" id="isi_konten" rows="12" 
                                                  class="w-full border-gray-300 rounded-xl shadow-sm focus:border-emerald-500 focus:ring-emerald-500 py-3 px-4 text-gray-700 transition duration-200 placeholder-gray-400 font-sans leading-relaxed"
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="bg-white overflow-hidden shadow-xl sm:rounded-2xl border border-gray-100 p-6 space-y-5">
                            ...
                            {{-- Kategori --}}
                            <div>
                                <label for="kategori_edukasi_id" class="block text-sm font-semibold text-gray-700 mb-2">Kategori</label>
                                <div class="relative">
                                    <select name="kategori_edukasi_id" id="kategori_edukasi_id" 
                                            class="w-full border-gray-300 rounded-xl shadow-sm focus:border-emerald-500 focus:ring-emerald-500 py-2.5 px-4 appearance-none bg-white">
                                        <option value="">-- Pilih Kategori --</option>
                                        @foreach ($kategori as $item)
                                            <option value="{{ $item->id }}" {{ old('kategori_edukasi_id', $konten->kategori_edukasi_id) == $item->id ? 'selected' : '' }}>
                                                {{ $item->nama_kategori }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            {{-- Tipe Konten --}}
                            <div>
                                <label for="tipe_konten" class="block text-sm font-semibold text-gray-700 mb-2">Tipe Konten</label>
                                <select name="tipe_konten" id="tipe_konten" 
                                        class="w-full border-gray-300 rounded-xl shadow-sm focus:border-emerald-500 focus:ring-emerald-500 py-2.5 px-4 bg-white"
                                        onchange="toggleVideoInput()">
                                    <option value="artikel" {{ old('tipe_konten', $konten->tipe_konten) == 'artikel' ? 'selected' : '' }}>📄 Artikel</option>
                                    <option value="video" {{ old('tipe_konten', $konten->tipe_konten) == 'video' ? 'selected' : '' }}>🎥 Video</option>
                                </select>
                            </div>
                            ...
                            <div id="video_url_container" class="{{ old('tipe_konten', $konten->tipe_konten) == 'video' ? 'block' : 'hidden' }}">
                                <label for="url_video" class="block text-sm font-semibold text-gray-700 mb-2">URL Youtube</label>
                                <div class="relative rounded-md shadow-sm">
                                    ...
                                    <input type="text" id="url_video" name="url_video" value="{{ old('url_video', $konten->url_video) }}" 
                                           class="focus:ring-emerald-500 focus:border-emerald-500 block w-full pl-10 sm:text-sm border-gray-300 rounded-xl" 
                        ...
                        <div class="bg-white overflow-hidden shadow-xl sm:rounded-2xl border border-gray-100 p-6">
                            <h3 class="text-lg font-semibold text-gray-800 border-b pb-2 mb-4">Media Sampul</h3>
                            
                            {{-- Foto Saat Ini --}}
                            <div class="mb-4">
                                <label class="block text-xs font-medium text-gray-500 uppercase tracking-wider mb-2">Foto Saat Ini</label>
                                @if($konten->foto_sampul)
                                    <div class="relative w-full h-40 rounded-lg overflow-hidden group shadow-md">
                                ...
                                @else
                                    <div class="w-full h-24 bg-gray-100 rounded-lg border-2 border-dashed border-gray-300 flex items-center justify-center text-gray-400">
                            ...
                            <div>
                                ...
                                <div class="flex items-center justify-center w-full">
                                    <label for="foto_sampul" class="flex flex-col items-center justify-center w-full h-32 border-2 border-emerald-300 border-dashed rounded-lg cursor-pointer bg-emerald-50 hover:bg-emerald-100 transition duration-300">
                                        <div class="flex flex-col items-center justify-center pt-5 pb-6">
                                            <svg class="w-8 h-8 mb-3 text-emerald-500" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12"></path></svg>
                                ...
                                <div id="preview-container" class="mt-4 hidden">
                                    ...
                                    <img id="preview-img" class="w-full h-32 object-cover rounded-lg shadow" />
                                </div>
                            </div>
                        </div>
